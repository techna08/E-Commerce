...
const orgPort = process.env.BACKEND_PORT || 3000;
...
app.all('/customers*',jsonParser, async (req, res) => {
  try {
    ...
    if (req.get('User-Agent').includes('Mobile') && req.method==='GET')  {
      ...
      delete responseData.address2;
      delete responseData.city;
      delete responseData.state;
      delete responseData.zipcode;
...
app.listen(PORT, () => {
  console.log(`Cust BFF listening on port ${PORT}`);
});